/* client.cpp
    ...
    Written by Giric Goyal for CS 450 HW3 April 2014
*/
    ...
    do {
        ...
        while (true) {
            ...
            if (c_nextSeqNumber < c_basePointer + c_windowSize && c_nextSeqNumber <= c_totalPackets) {
                ...
                c_fileName.c_str(), c_clientAddr.sin_port, c_serverAddr.sin_port, htons(checksum), 3 /*HW no*/, packetType, 
                relayCommand,  saveFile, "ggoyal2", dropChance, dupeChance, garbleChance, delayChance, c_windowSize, 31 /*version*/);
            ...
            if (c_retVal == -1) {
                ...
                if (recvLen < 0) {
                    ...
                    else if (checksum == 0 && c_basePointer <= ntohl(c_packet->header.ackNumber)) {

                        std::cout<<"\nAck number : "<<ntohl(c_packet->header.ackNumber)<<" received.\n";
                        
                        // if not corrupt.
                        c_basePointer = ntohl(c_packet->header.ackNumber) + 1;

                        //cout<<c_basePointer<<endl;
            ...
            if (c_timerFlag) {
                ...
                // send all
                for (int i = c_basePointer; i < c_nextSeqNumber; i++) {

                    dataSize = BLOCKSIZE;
            
                    if (i == c_totalPackets) {
                        dataSize = c_st.st_size - ((i - 1) * BLOCKSIZE);
                    }
                     
                    strncpy(c_tempString, (char*)c_addr + ((i - 1) * BLOCKSIZE), dataSize);
                    ...
                    // create packet
                    packHeader(c_header, htonl(7) , htonl(657440995), htonl(transactionNumber),
                    htonl(i), htonl(0), c_clientAddr.sin_addr.s_addr, c_serverAddr.sin_addr.s_addr,
                    c_clientAddr.sin_addr.s_addr, c_serverAddr.sin_addr.s_addr, htonl(nbytes), htonl(nTotalBytes), 
                    c_fileName.c_str(), c_clientAddr.sin_port, c_serverAddr.sin_port, htons(checksum), 3 , packetType, 
                    relayCommand,  saveFile, "ggoyal2", dropChance, dupeChance, garbleChance, delayChance, c_windowSize, 31);
                    ...
                    if (sent < 0) {
                        ...
                        std::cout<<"Packet with seq number : "<<i<<" RESENT.\n";
                    }
                }
